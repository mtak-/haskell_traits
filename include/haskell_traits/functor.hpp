...
HASKELL_TRAITS_BEGIN
    ...
    auto fmap(T && t, F && f) DECLTYPE_NOEXCEPT_RETURNS(Impl::fmap((T &&) t, (F &&) f));

    template<typename T, typename F = unary_func_t<nonesuch2, const bound_to<T>&>>
...
HASKELL_TRAITS_DETAIL_BEGIN
    ...
    using functor_of_
        = std::enable_if_t<functor<T> && std::is_same_v<uncvref<bound_to<T>>, uncvref<C>>>;
HASKELL_TRAITS_DETAIL_END

HASKELL_TRAITS_BEGIN
    template<typename T, typename C>
    inline constexpr auto functor_of = is_detected<detail::functor_of_, T, C>;
HASKELL_TRAITS_END
...
#define FUNCTOR_ASSERTS(...)                                                                       \
    ...
    static_assert(::haskell_traits::fmappable<                                                     \
                      __VA_ARGS__,                                                                 \
                      ::haskell_traits::unary_func_t<::haskell_traits::nonesuch2,                  \
    ...
    static_assert(::haskell_traits::fmappable<                                                     \
                      __VA_ARGS__,                                                                 \
                      ::haskell_traits::unary_func_t<const ::haskell_traits::nonesuch2&,           \
                                                     ::haskell_traits::bound_to<__VA_ARGS__>>>,    \
                  #__VA_ARGS__ " does not correctly implement functor_impl::fmap");                \
    static_assert(::haskell_traits::fmappable<                                                     \
                      __VA_ARGS__,                                                                 \
